...
var chartGraphColor = [	"#ECD078",		// 차트색 (총 5종)
               			...
               			"#53777A" ];
*/
var chartGraphColor = [	"#4472C4",		// 차트색 (총 5종)
               			"#DAE3F3",
               			"#FD5554",
               			"#FE54FC",
               			"#52FC54" ];
...
function fn_egov_initl_mobilechartgraphlist(){
	...
	$.getJSON(contextPath + "/mbl/com/mcg/selectMobileChartGraphList.mdo", function(json) {
		...
		for (var i = 0; i < totcnt-1; i++) {
			for(var j = totcnt - 1; j > i; j--){
				if(	( resultList[j-1].lgdNm > resultList[j].lgdNm ) ||
					...
					( (resultList[j-1].lgdNm == resultList[j].lgdNm)
						&&(resultList[j-1].xaxis == resultList[j].xaxis)
						&&(resultList[j-1].yaxis > resultList[j].yaxis) )
				){
...
function fn_egov_draw_linechartgraph(){
	...
	var y_value_interval = 5;										// Y축 눈금 값의 간격
...
function fn_egov_draw_piechartgraph(){
	...
	for (var i = 0; i < pie_data.length && i < maxChartGraph; i++) {
		...
		context.fillStyle = chartGraphColor[i];
		if(i == 0){context.fillStyle = "#DAE3F3"}
		if(i == 1){context.fillStyle = "#DAE3F3"}
		
		context.strokeStyle = line_color;
		if(i == 0){context.strokeStyle = "#4472C4"}
		if(i == 1){context.strokeStyle = "#DAE3F3"}
		//context.lineWidth = line_width;
